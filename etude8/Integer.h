...
namespace cosc326 {
	...
	public:
		const std::vector<int>& getNum() const;//accessor.
		unsigned int getElements() const; //accessor.
	...
	private:
		...
		std::vector<int> number; //Vectors are the same as dynamic arrays thus appropriate for representing numbers.
		unsigned int elements; //this is needed to store values from the size() function which returns the numbers from the set container
	};
